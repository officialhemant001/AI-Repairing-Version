...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      ...
      try {
        ...
        // Assuming paginated response with 'results' array
  ...
  // Roughly estimate 1 hour per repair attempt
  const timeSaved = history.length; 

  const recentScans = history.slice(0, 5); // Just show top 5 on dashboard
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-in fade-in duration-300">
      ...
      <div>
        <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4">
          ...
          {history.length > 0 && (
             <button onClick={() => navigate('/history')} className="text-sm text-blue-400 hover:text-blue-300 font-medium flex items-center gap-1 transition-colors w-fit">
              ...
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-slate-700/50">
                  {recentScans.length > 0 ? recentScans.map((scan) => {
                    ...
                    return (
                      <tr key={scan.id} className="hover:bg-slate-700/30 transition-colors group cursor-pointer" onClick={() => navigate(`/scan/${scan.id}`)}>
                        <td className="px-6 py-5">
                          <div className="font-semibold text-slate-200">{scan.issue || 'Unknown Issue'}</div>
                          <div className="text-sm text-slate-400 capitalize mt-0.5 tracking-wide">{scan.appliance_category?.replace('_', ' ') || 'General'} • {scan.input_type || 'Image'}</div>
                        </td>
                        <td className="px-6 py-5">
                        ...
                        <td className="px-6 py-5 text-right">
                          <button 
                            className="text-sm font-semibold text-blue-400 group-hover:text-blue-300 transition-colors flex items-center justify-end gap-1 w-full"
                          >
