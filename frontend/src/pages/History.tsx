...
import { Camera, Clock, ArrowLeft, Filter, Search, Heart, Bookmark } from 'lucide-react';
...
const History = () => {
  ...
  const filteredHistory = useMemo(() => {
    ...
    if (searchTerm.trim()) {
      ...
      result = result.filter(scan => 
        ...
        (scan.device_name && scan.device_name.toLowerCase().includes(term)) ||
        (scan.category_name && scan.category_name.toLowerCase().includes(term))
      );
    }

    if (showOnlyFavorites) {
      result = result.filter(scan => scan.is_favorite);
    }

    if (showOnlyBookmarks) {
      result = result.filter(scan => scan.is_bookmarked);
    }

    if (selectedSeverity) {
      result = result.filter(scan => scan.severity === selectedSeverity);
    }

    return result;
  }, [searchTerm, history, showOnlyFavorites, showOnlyBookmarks, selectedSeverity]);
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-in fade-in duration-300">
      ...
      {/* Search & Filter Bar */}
      <div className="bg-slate-800/60 backdrop-blur-md border border-slate-700/50 rounded-2xl p-4 mb-8 shadow-sm flex flex-col gap-4">
        <div className="flex flex-col sm:flex-row gap-4">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="Search by issue or device model..."
              className="block w-full pl-10 pr-3 py-2.5 border border-slate-600 rounded-xl bg-slate-900/50 text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all text-sm"
            />
          </div>
          ...
          <select
            ...
            className="bg-slate-700 text-white border border-slate-600 px-4 py-2.5 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm font-medium"
          >
        ...
        {/* Favorites and bookmarks filters */}
              ...
              return (
                ...
                >
                   <div className="flex justify-between items-start mb-3">
                      <span className="text-xs font-semibold uppercase tracking-wider text-slate-400 bg-slate-900/80 px-2.5 py-1 rounded-md border border-slate-700">
                        {scan.category_icon} {scan.category_name || scan.appliance_category?.replace('_', ' ') || 'General'}
                      </span>
                   ...
                   <p className="text-sm text-slate-400 line-clamp-2 mb-4 leading-relaxed font-medium">
                     {scan.description || scan.root_cause || 'No description provided.'}
                   </p>
